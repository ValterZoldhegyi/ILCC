...
def vis_segments_only_chessboard_color(ind=1):
    ...
    chessboard_file_name = \
        cPickle.load(open(os.path.join(params['base_dir'], "output/pcd_seg/") + str(ind).zfill(
            params["file_name_digits"]) + "_pcd_result.pkl", "rb"))[
            -1].split("/")[-1]
    for seg in seg_list:
        if seg.split(".")[-1] == "txt":
            if seg in chessboard_file_name:
...
def calculate_reprojection_error(ind_ls = (np.arange(1, params['poses_num']+1)).tolist()):
    ...
    #rotation matrix calculation for point cloud data
    ...
    for i in ls:
        ...
        transformed_pcd = np.dot(rot_mat, corners_in_pcd_arr.T).T + r_t[3:]  # point cloud in the camera coordinate system
        transformed_pcd_ls = corners_in_pcd_arr.tolist()  # original point cloud in a list

        if params["camera_type"] == "panoramic":
            pcd2angle_s = map(xyz2angle, transformed_pcd_ls) #calculate pcd points from xyz to polar angles
        ...
        proj_pts = (pcd_to_pix / pcd_to_pix[:, 2].reshape(-1, 1))[:, :2].astype(np.int16)
        if np.linalg.norm(np.array(proj_pts[0]) - np.array([0, H])) > np.linalg.norm(
                np.array(proj_pts[-1]) - np.array([0, H])):
            proj_pts = np.flipud(proj_pts)
        error = np.mean(np.linalg.norm(corners_in_img_arr-proj_pts, axis=1))
        error_ls.append(error)

    objects = ls
    y_pos = np.arange(len(ls))
    performance = np.array(error_ls)
...
def back_project_pcd(img, pcd_arr, color_arr, r_t, i, hide_occlussion_by_marker):
    ...
    rot_mat = np.dot(transforms3d.axangles.axangle2mat([0, 0, 1], r_t[2]),
                     np.dot(transforms3d.axangles.axangle2mat([0, 1, 0], r_t[1]),
                            transforms3d.axangles.axangle2mat([1, 0, 0], r_t[0])))
    transformed_pcd = np.dot(rot_mat, pcd_arr.T).T + r_t[3:] #point cloud in the camera coordinate system

    transformed_pcd_ls = pcd_arr.tolist() #original point cloud in a list
    ...
    if not hide_occlussion_by_marker:  # whether remove occlussions by the chessboard
        ...
        chessboard_result_file_path = os.path.join(params['base_dir'], "output/pcd_seg/" + str(i).zfill(
            params["file_name_digits"]) + "_pcd_result.pkl")
        chessboard_result_file = cPickle.load(open(chessboard_result_file_path, "rb"))
...
def vis_back_proj(ind=1, img_style="edge", pcd_style="intens", hide_occlussion_by_marker=False,
    ...
    if pcd_style == "intens":
        ...
        pcd_color = np.fliplr((cm.jet(dis_arr / 42) * 255).astype(np.int32)[:, :3])
    else:
